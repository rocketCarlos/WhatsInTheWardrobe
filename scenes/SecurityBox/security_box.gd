...
Copy paste from GenericContainer (lol) to implement a security box that requires a numerical and a
symbolic code to be opened
'''
 #region scene nodes
...
func restart() -> void:
	...
	if centered == false:
		hitbox.position += Vector2(sprite_closed.get_width()/2, sprite_closed.get_height()/2)
	...
	if Input.is_action_just_pressed("Interact"):
		if (open and mouse_in_open) or (not open and mouse_in_closed):
			if open: # switching from open to closed
				...
				Globals.main.show_security_box(self)
